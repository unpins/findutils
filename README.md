...
```bash
unpin findutils --unpin-program=find . -name '*.md' -newer Makefile
unpin findutils --unpin-program=find /var/log -type f -mtime +30 -delete
echo file1 file2 file3 | unpin findutils --unpin-program=xargs rm
```
...
`find.1` and `xargs.1` are embedded in the binary — read any with `unpin man findutils <program>`, e.g. `unpin man findutils find`.
